...
float World::Updater(Mario &hero, Dungeonarea &maps, Spawner &vectors, sf::RectangleShape &player, sf::View &view1,
    ...
    for (auto &gl: skill) {
        ...
        if (gl->isTargetLost() == false || gl->getHp() <= 0) {
            ...
            if (gl->getHp() > 0 &&
                (l2Distance(*gl->getTarget(), gl->getposX() + gl->getDirectX(), gl->getposY() + gl->getDirectY()) !=
                 0) &&
...
bool World::initialize(Mario &hero, int &mapIndex, bool &tutorialItem, bool &tutorialSafezone, bool &tutorialTeleport,
