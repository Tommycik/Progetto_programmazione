...
Dungeonarea::Dungeonarea(long oldseed, int maxLength, int maxHeigth, int minRoomWidth, int minRoomHeight,
                         ...
                         std::string save, int minRooms) : xMax(maxLength), yMax(maxHeigth), minRoomWidth(minRoomWidth),
                                                           minRoomHeight(minRoomHeight),
                                                           parts(parts), xMin(xMin), yMin(yMin),
                                                           maxRoomHeight(maxRoomHeight), maxRoomWidth(maxRoomWidth) {
...
bool Dungeonarea::loadMap(std::string fileName, std::string name) {
        ...
        for (int x = 0; x < width; x++) {
            ...
            try {
                ...
                throw std::out_of_range(
                        "Can not set map tile at x: " + std::to_string(x) + " - y: " + std::to_string(y));
            }
    ...
    if (y != height)
        throw std::runtime_error(
                "Map height size wrong. Expected " + std::to_string(height) + " but read: " + std::to_string(y));
