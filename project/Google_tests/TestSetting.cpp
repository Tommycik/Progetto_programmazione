...
bool
TheClassTest::draw(Mario &hero, sf::View &view1, sf::RectangleShape &player, Hud &hud, Textviewer &objectInteraction,
...
bool TheClassTest::creation(std::unique_ptr<Spawner> *vectors, std::unique_ptr<Dungeonarea> *maps) {
    ...
    try {
        for (int i = 0; i < numberMap; i++) {
            ...
            maps[i] = std::make_unique<Dungeonarea>(oldseed, minRoomsNumber * 20 + 1, minRoomsNumber * 20 + 1, 15, 15,
                                                    30, 30, i, minRoomsNumber * 10, minRoomsNumber * 15,
                                                    minRoomsNumber * 15, names[i], saves[i], minRoomsNumber);
