...
bool Textviewer::blackBox(float posX,float posY,std::string text1, std::string text2,sf::RenderWindow *window,bool fullScreen ) {
...
void Textviewer::show(sf::RenderWindow &window,sf::View &view1, bool &makeText, bool &itemText, bool &tutorialItem, bool &safezoneText,
    ...
    if(makeText) {
        if(itemText){
            ...
            if(tutorialTeleport){
                this->blackBox(box.getPosition().x,box.getPosition().y,"Il teletrasporto emana un'aura misteriosa...","",&window,false);
            }else {
                this->blackBox(box.getPosition().x,box.getPosition().y,"Per usare il potere del teletrasporto premere O o P","",&window,false);
            }
        }
    }
}
