...
int main() {
//todo togliere metodi inutili nelle varie classi e ripulire da main in poi
//todo inizializzare variabili nelle classi
    ...
    game.creation(*hero, monsterNumber, objectNumber, safezoneNumber, bossNumber, numberMap, oldseed, minRooms,
                  mapIndex, tutorialItem, tutorialSafezone, tutorialTeleport,
    ...
    if (!game.initialize(*hero, mapIndex, tutorialItem, tutorialSafezone, tutorialTeleport, HudBarsHeigth, numberMap,
    ...
    if (!map.loadMap("assets/Textures-16.png", sf::Vector2u(tilesetResolution, tilesetResolution), *maps[mapIndex],
                     maps[mapIndex]->getWidth(), maps[mapIndex]->getHeight())) {
        return -1;
    }
    bool change = false;
    while (window.isOpen()) {
        ...
        while (window.isOpen()) {
            ...
            switch (eventControl) {
                ...
                case 2:
                    if (!map.loadMap("assets/Textures-16.png", sf::Vector2u(tilesetResolution, tilesetResolution),
                                     *maps[mapIndex], maps[mapIndex]->getWidth(), maps[mapIndex]->getHeight())) {
